...
def display_ci(interval):
    if not interval or interval.get("low") is None or interval.get("high") is None:
        return "N/A"
    return f"{display_percent(interval['low'])} to {display_percent(interval['high'])}"

def build_report_payload(run_id, results, evaluation_summary):
    return {
        "run_id": run_id,
        "total": evaluation_summary["total"],
        "correct": evaluation_summary["correct"],
        "wrong": evaluation_summary["wrong"],
        "accuracy": value_as_percent(evaluation_summary["overall_accuracy"]),
        "results": results,
        "classes": CLASS_NAMES,
        "evaluation": evaluation_summary
    }

def store_evaluation_report(conn, run_id, report_payload):
    evaluation = report_payload["evaluation"]
    averages = evaluation.get("averages", {})
    ...
    ''', (
        evaluation["total"],
        value_as_percent(evaluation["overall_accuracy"]),
        value_as_percent(averages.get("macro_f1")),
        evaluation["correct"],
        evaluation["wrong"],
        value_as_percent(averages.get("weighted_f1")),
        value_as_percent(averages.get("balanced_accuracy")),
        json.dumps(report_payload),
        run_id
    ))

def get_report_payload(run_id):
    conn = get_db()
    row = conn.execute(
        "SELECT report_json FROM test_runs WHERE id = ?",
        (run_id,)
    ).fetchone()
    conn.close()
...
def make_report_pdf(report_payload):
    chrome_path = find_chrome_executable()
    if not chrome_path:
        raise RuntimeError("Chrome or Edge is required to generate the styled PDF report.")
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        if completed.returncode != 0 or not os.path.exists(pdf_path):
            ...
            raise RuntimeError(f"Failed to generate PDF report: {stderr}")
...
def api_evaluate():
    ...
    conn = get_db()
    cursor = conn.cursor()
    cursor.execute("INSERT INTO test_runs (date_run, total_images, accuracy, macro_f1) VALUES (?, 0, 0, 0)", (run_date,))
    ...
    report_payload = build_report_payload(run_id, results, evaluation_summary)
    store_evaluation_report(conn, run_id, report_payload)

    for res in results:
        cursor.execute('''
            INSERT INTO test_images (run_id, filename, expected, prediction, confidence, is_correct)
            VALUES (?, ?, ?, ?, ?, ?)
        ''', (run_id, res["filename"], res["expected"], res["prediction"], res["confidence"], res["correct"]))
...
def api_get_test_runs():
    ...
    rows = conn.execute('''
        SELECT
            ...
            weighted_f1,
            balanced_accuracy,
            CASE WHEN report_json IS NULL THEN 0 ELSE 1 END AS report_available
        FROM test_runs
        ORDER BY id DESC
    ''').fetchall()
    conn.close()
...
@app.route("/api/test_images/<int:run_id>", methods=["GET"])
def api_get_test_images(run_id):
    conn = get_db()
    rows = conn.execute("SELECT * FROM test_images WHERE run_id = ?", (run_id,)).fetchall()
    conn.close()
    return jsonify({"images": [dict(row) for row in rows]})

@app.route("/api/test_runs/<int:run_id>/report", methods=["GET"])
def api_get_test_run_report(run_id):
    report_payload = get_report_payload(run_id)
    if not report_payload:
        return jsonify({"error": "Report not found for this run"}), 404
    return jsonify(report_payload)

@app.route("/api/test_runs/<int:run_id>/pdf", methods=["GET"])
def api_download_test_run_pdf(run_id):
    report_payload = get_report_payload(run_id)
    if not report_payload:
        return jsonify({"error": "Report not found for this run"}), 404

    try:
        pdf_bytes = make_report_pdf(report_payload)
    except RuntimeError as exc:
        return jsonify({"error": str(exc)}), 500

    filename = f"banana-model-evaluation-report-{run_id}.pdf"
    return Response(
        pdf_bytes,
